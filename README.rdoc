...
Simplest form, use the default credentials to copy files to s3 or back.
    ...
    Xeroxer.copy("file://path/path/path/file.ext", "s3://bucket.s3.amazon.com/path/path/path/file.ext"
...
You can set credentials to use to copy to and from a particular bucket.  If you use this bucket with specified credentials in an s3:// url, Xeroxer will use the credentials you have specified for that bucket and not the default one.       NOT WORKING YET!
    ...
    Xeroxer::configure({:bucketname => {:s3_credentials=> {:access_key_id => your_key, :secret_access_key => your_secret}}})

Or you can pass everything in one go: (Passed in credentials to a Xeroxer.copy will override any other credentials given. )

    Xeroxer.copy("file://path/path/path/file.ext", "s3://bucket.s3.amazon.com/path/path/path/file.ext", {:s3_credentials=> {:access_key_id => your_key, :secret_access_key => your_secret}})

You can also copy files around locally, no credentials required, assumed read/write privileges:

    Xeroxer.copy("file://path/path/path/file1.ext", "file://path/path/path/file2.ext"

NOT IMPLEMENTED YET:

    Xeroxer.copy("file://path/path/path/file1.ext", "http://url/path/path/path/file2.ext")
    Xeroxer.copy("http://url/path/path/path/file1.ext", "file://path/path/path/file2.ext")
    Xeroxer.copy("s3://bucket.s3.amazon.com/path/path/path/file.ext", "http://url/path/path/path/file2.ext")
    Xeroxer.copy("http://url/path/path/path/file2.ext", "s3://bucket.s3.amazon.com/path/path/path/file.ext")
    Xeroxer.copy("http://url/path/path/path/file2.ext", "http://url/path/path/path/file2.ext")
    Xeroxer.copy("s3://bucket.s3.amazon.com/path/path/path/file.ext", "s3://bucket.s3.amazon.com/path/path/path/file.ext")
